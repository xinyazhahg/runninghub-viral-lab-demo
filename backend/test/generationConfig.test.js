...
test("persists a submitted 4s generation without falling back to 10s", () => {
  ...
  assert.equal(persisted.targetDuration, 4);
...
test("persists a submitted 10s generation and explicit clip range", () => {
  const finalGenerationConfig = buildFinalGenerationConfig(config(10));
  const persisted = buildPersistedGenerationConfig(finalGenerationConfig, { clipStart: "2", clipEnd: "8" });
  assert.equal(finalGenerationConfig.duration, 10);
...
test("the 10s final config is reused by Task and Result model_params without a stale 4s value", () => {
  const finalGenerationConfig = buildFinalGenerationConfig(config(10));
  const persisted = buildPersistedGenerationConfig(finalGenerationConfig, { clipStart: 0, clipEnd: 10 });
  const input = buildGenerationTaskInput({ generationConfig: persisted });
  const resultModelParams = persisted;
  assert.equal(input.duration, 10);
  assert.equal(input.model_params.duration, 10);
  assert.equal(resultModelParams.duration, 10);
  assert.doesNotMatch(JSON.stringify({ input, resultModelParams }), /"duration":4(?:[,}])/);
});
...
test("persists the complete canonical Task input and replacement Asset references", () => {
  ...
  const input = buildGenerationTaskInput({
    ...
    generationConfig, clipStart: 0, clipEnd: 4, promptDuration: 10,
  });
  assert.equal(input.duration, 4);
  assert.equal(input.targetDuration, 4);
  assert.equal(input.promptDuration, 10);
