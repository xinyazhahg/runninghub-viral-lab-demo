...
test('Prompt 成功后才创建并聚焦视频生成节点，失败分支不会创建空节点', () => {
  ...
  assert.match(app, /generationNodeVisible = computed\(\(\) => promptDraft\.status !== 'idle'\)/)
...
test('生成按钮只受生成态、Prompt、素材上传和配置完整性约束，正常态为绿色', () => {
  ...
  assert.match(disabled, /isGenerating[\s\S]*isGeneratingPrompt[\s\S]*referenceUploading[\s\S]*configReady[\s\S]*promptValue/)
